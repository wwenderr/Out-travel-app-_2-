...
public struct StageOneViewModel {
    ...
    @discardableResult
    public mutating func attachPhotoURL(_ url: String, toPlace placeId: UUID) throws -> Place {
        try store.addPhotoURL(url, toPlace: placeId)
    }

    @discardableResult
    public mutating func detachPhotoURL(_ url: String, fromPlace placeId: UUID) throws -> Place {
        try store.removePhotoURL(url, fromPlace: placeId)
    }
}
